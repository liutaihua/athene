...
def process_line(l):
    ...
    sql = """INSERT INTO quest_log (`userid`,`timestamp`,`key`,`quest_id`,`add_exp`,`add_gold`,`add_attr1`,`add_attr2`,`add_attr3`,`add_attr4`,`add_attr5`,`add_attr6`,`add_attr7`,`add_attr8`,`add_attr9`,`add_attr10`,`new_attr1`,`new_attr2`,`new_attr3`,`new_attr4`,`new_attr5`,`new_attr6`,`new_attr7`,`new_attr8`,`new_attr9`,`new_attr10`,`new_gold`,`new_exp`,`reward_item`, `time_tail`, `serverid`) VALUES (%s,'%s','%s','%s',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'%s', %s, %s)""" % (
        ...
        d.get("new_gold", 0), d.get("new_exp", 0), d.get("reward_item", " "), d.get('time', '').split('.')[-1], d.get('userid', 0)%65536)
    try:
    ...
    for field in ['attr1', 'attr2', 'attr3', 'attr4', 'attr5', 'attr6', 'attr7', 'attr8', 'attr9', 'attr10',
        ...
        if int(d.get('add_%s' % field, 0)) != 0:
            ...
            sql = """INSERT INTO attr_log (userid, attr, timestamp, old_data, delta_type, delta_data, source, time_tail, serverid) VALUES (%s, '%s', '%s', %s, '%s', %s, '%s', %s, %s)""" % (
                d.get("userid", 0), field.replace('attr', 'attr_'), time, old_data, delta_type, delta_data, source, d.get('time', '').split('.')[-1], d.get('userid', 0)%65536
            )
            try:
                #print sql
                common.conn.execute(sql)
